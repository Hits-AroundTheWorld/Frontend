    ...
    <Card border="light" className="mt-2">
      <Card.Body>
        <Row className="px-3 d-flex justify-content-between" >
          ...
          <Card.Subtitle>{trip.peopleCountNow}/{trip.maxPeopleCount}</Card.Subtitle>
        </Row>

        <Card.Subtitle className="mb-2 text-muted">{new Date(trip.startDate).toLocaleDateString()} - {new Date(trip.endDate).toLocaleDateString()}</Card.Subtitle>
        <Card.Text className="mb-1">Описание: {trip.tripMiniDescription}</Card.Text>
        <Card.Text className="mb-1">Организатор: {trip.tripFounderFullName}</Card.Text>
        <Card.Text className="mb-1">Максимальный бюджет: {trip.maxBudget}₸</Card.Text>
        <Row className="px-3">
          <Card.Text className="mb-1">Пригласить: </Card.Text>
          <OverlayTrigger placement="top" overlay={<Tooltip id="button-tooltip">Нажмите для копирования</Tooltip>}>
            <Card.Text className="mb-1" onClick={() => {navigator.clipboard.writeText(trip.invitationLink)}} style={{ cursor: 'pointer' }}>
              {trip.invitationLink}
            </Card.Text>
          </OverlayTrigger>
        </Row>
